...
class ClaudeFrameworkSetup:
					...
					app_info.update(
						{
							...
							"app_publisher": project.get("authors", [{}])[0].get("name", "")
							if project.get("authors")
							else "",
							"app_email": project.get("authors", [{}])[0].get("email", "")
							if project.get("authors")
							else "",
						}
					)
	...
	def create_backup(self) -> str:
		...
		critical_files = ["CLAUDE.md", "docs/", "hooks.py", ".github/workflows/", ".pre-commit-config.yaml"]
...
**APP:** {{APP_NAME}} v1.0.0"""
	...
	def _replace_placeholders(self, content: str, config: dict) -> str:
		...
		replacements = {
			...
			"INITIAL_MODULES": config["modules"]["initial_modules"]
			if isinstance(config["modules"]["initial_modules"], str)
			else ", ".join(config["modules"]["initial_modules"]),
			"MODULE_COUNT": str(
				len(config["modules"]["initial_modules"])
				if isinstance(config["modules"]["initial_modules"], list)
				else 1
			),
			"CURRENT_DATE": datetime.now().strftime("%Y-%m-%d"),
			"REQUIRED_APPS": config["dependencies"]["required_apps"]
			if isinstance(config["dependencies"]["required_apps"], str)
			else ", ".join(config["dependencies"]["required_apps"]),
		}
...
if __name__ == "__main__":
	...
	def validate_setup(self) -> list[str]:
		...
		critical_files = ["CLAUDE.md", "docs/core/CLAUDE_CONFIG.md", "docs/operational/MODULE_STATUS.md"]
	...
	def run_setup(self, config: dict, create_backup: bool = True) -> bool:
		...
		try:
			...
			print("📁 Configuración guardada en: claude_framework_config.json")
			print("📖 Documentación principal: CLAUDE.md")
