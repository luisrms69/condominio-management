#!/usr/bin/env python3
"""
REGLA #53 - Budget Planning Layer 4 Type B Performance Test
Categoría B: Budget Calculation Performance validation - Target: < 250ms
"""
...
class TestBudgetPlanningL4TypeB(FrappeTestCase):
	...
	def test_budget_calculation_performance(self):
		"""Test: Budget Calculation Performance - Target: < 250ms (REGLA #53)"""
		# REGLA #53: Performance test crítico para Budget Planning
		...
		try:
			...
			self.assertLess(
				...
				f"{self.doctype} Budget Calculation Performance took {execution_time:.3f}s, target: {self.performance_target}s",
			)

			# 5. Validate operation success
			self.assertIsNotNone(result, f"{self.doctype} Budget Calculation Performance must return result")
			...
			self.assertLess(
				execution_time,
				self.performance_target,
				f"{self.doctype} Budget Calculation Performance took {execution_time:.3f}s even with error: {e}",
			)
