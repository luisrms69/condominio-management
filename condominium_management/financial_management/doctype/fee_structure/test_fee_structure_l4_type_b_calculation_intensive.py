...
class TestFeeStructureL4TypeBCalculationIntensive(FrappeTestCase):
	...
	def _execute_calculation_intensive_operation(self, test_config):
		...
		try:
			...
			for i in range(test_config["calculation_count"]):
				...
				savings_vs_highest = max(fee_options, key=lambda x: x["amount"])["amount"] - optimal_method["amount"]
	...
	def _validate_calculation_intensive_performance(self, result, execution_time):
		...
		if result and result.get("status") == "Calculation Intensive Success":
			...
			self.assertGreater(result["avg_optimal_amount"], 0, "Calculation intensive must calculate optimal amounts")
			self.assertGreaterEqual(result["total_savings"], 0, "Calculation intensive must calculate savings")
			self.assertIsInstance(result["method_distribution"], dict, "Calculation intensive must track method distribution")
